...
const Q = {
  ...
  /**
   * THE REFERRAL LOOP, against the names the app ACTUALLY emits.
   *
   * This asked for 'referral_code_applied', which nothing has ever sent — the app's event is
   * 'referral_redeemed' — so the last step of the loop was a permanent zero. It also counted
   * 'share_call_tapped', which is sharing a CALL RECORDING, a different action from inviting
   * someone. Both are the silent-zero failure this file warns about elsewhere: a query that runs,
   * returns a confident number, and measures nothing.
   *
   * Shares are split tapped/completed because the app splits them: the sheet opening is intent,
   * Share.sharedAction is the thing actually sent, and counting only the first calls everyone who
   * backed out a referrer.
   */
  referralLoop: `SELECT
      uniqIf(person_id, event='$screen' AND properties.$screen_name='referral') AS opened,
      uniqIf(person_id, event='referral_share')           AS shared,
      uniqIf(person_id, event='referral_share_completed') AS sent,
      uniqIf(person_id, event='referral_redeemed')        AS redeemed
    FROM events WHERE timestamp >= now() - INTERVAL 90 DAY`,

  /**
   * k, DERIVED FROM EVENT HISTORY — works today, for everyone, retroactively.
   *
   * The first version of this read a person property (activated_on) the app only started stamping
   * this week, so the base would have read near-zero until adoption caught up and the card would
   * have said "not enough base yet" for three months. That is a measurement you cannot use, which
   * is the same problem as the one it replaced.
   *
   * The 'activated' EVENT has been firing since the beginning, so each person's activation date is
   * already in the stream: min(timestamp) where event='activated'. Deriving it there needs no new
   * property, covers every user who ever activated, and is correct the moment it ships.
   *
   * The denominator is then the population the numerator can actually be drawn from: people who
   * had already activated when the window opened. Someone who activated last week cannot have
   * referred anyone in the last 90 days, and counting them only makes k look worse for growing.
   */
  referralK: `SELECT
      countIf(act > toDateTime('1971-01-01'))                                       AS base_all_time,
      countIf(act > toDateTime('1971-01-01') AND act <= now() - INTERVAL 90 DAY)    AS base_in_window,
      countIf(redeemed_90 > 0)                                                      AS redemptions
    FROM (
      SELECT person_id,
             minIf(timestamp, event = 'activated') AS act,
             countIf(event = 'referral_redeemed' AND timestamp >= now() - INTERVAL 90 DAY) AS redeemed_90
      FROM events
  ...
  /**
   * COHORT k — by activation month, also derived from the stream.
   *
   * Each cohort is measured over ITS OWN first 30 days, which is what makes months comparable: a
   * cohort that joined last week has not had the same chance as one from March, and comparing them
   * against a shared window is the same mistake as the all-time denominator.
   *
   * `referred` counts people whose redemption landed within 30 days of their own activation. That
   * is the referee side — the only side visible retroactively, because referrer_code has only been
   * on redemptions since this week. Once that data accumulates, per-REFERRER cohort k becomes
   * possible; until then this is the honest half, and it is labelled as such on the card.
   */
  referralCohort: `SELECT
      formatDateTime(toStartOfMonth(act), '%Y-%m') AS cohort,
      count()                                      AS joined,
      countIf(red > toDateTime('1971-01-01') AND dateDiff('day', act, red) <= 30) AS referred
    FROM (
      SELECT person_id,
             minIf(timestamp, event = 'activated')        AS act,
             minIf(timestamp, event = 'referral_redeemed') AS red
      FROM events
      WHERE timestamp >= now() - INTERVAL 400 DAY
      GROUP BY person_id
    )
    WHERE act > toDateTime('1971-01-01')
...
async function build() {
  {
    ...
    return {
      ...
      referralLoop: (() => {
        ...
        const [opened, shared, sent, redeemed] = (r || []).map((x) => Number(x) || 0);
        return {
          opened, shared, sent, redeemed,
          shareRate: opened ? Math.round((shared / opened) * 1000) / 10 : null,
          sentRate: shared ? Math.round((sent / shared) * 1000) / 10 : null,
          redeemRate: sent ? Math.round((redeemed / sent) * 1000) / 10 : null,
        };
      })(),
      referralK: (() => {
        const [r] = R.referralK || [];
      ...
      referralCohort: (R.referralCohort || []).map(([cohort, joined, referred]) => {
        const j = Number(joined) || 0, rf = Number(referred) || 0;
        return {
          cohort, joined: j, referredSomeone: rf,
          pct: j ? Math.round((rf / j) * 1000) / 10 : null,
          // A month younger than 30 days has not finished its own window — show it, but never
          // compare it: a partial cohort always looks worse than a complete one.
          partial: (Date.now() - new Date(`${cohort}-01T00:00:00Z`).getTime()) < 60 * 86400000,
        };
      }),
