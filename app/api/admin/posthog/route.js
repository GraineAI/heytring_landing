...
const Q = {
  ...
  /**
   * k, WITH A DENOMINATOR THAT MATCHES THE WINDOW.
   *
   * The old figure divided redemptions in a 90-day window by all-time activated users. The 75
   * included people who activated inside the window — or last week — who never had 90 days to
   * refer anyone: they pad the denominator without a fair shot at the numerator. That understates
   * yield rather than flattering it, and it drifts whenever the base grows, for reasons that have
   * nothing to do with the loop.
   *
   * The app now stamps activated_on (a date, set once) on the person, so the base can be
   * restricted to people who were already activated when the window opened — the same population
   * the numerator is drawn from.
   */
  referralK: `SELECT
      uniqIf(person_id, event='referral_redeemed' AND timestamp >= now() - INTERVAL 90 DAY) AS redemptions,
      uniqIf(person_id, person.properties.activated_on != ''
             AND toDate(person.properties.activated_on) <= toDate(now() - INTERVAL 90 DAY))  AS base_in_window,
      uniqIf(person_id, person.properties.activated_on != '')                                AS base_all_time
    FROM events WHERE timestamp >= now() - INTERVAL 180 DAY`,

  /**
   * COHORT k — referrals generated by the people who joined in month N.
   *
   * The only form comparable month over month, because every cohort is measured over its own
   * span rather than against a base that keeps moving. Needs referrer_code on the redemption,
   * which the app now sends.
   */
  referralCohort: `SELECT
      substring(person.properties.activated_on, 1, 7) AS cohort,
      uniq(person_id) AS joined,
      uniqIf(person_id, person.properties.referrals_completed > 0) AS referred_someone
    FROM events
    WHERE person.properties.activated_on != '' AND timestamp >= now() - INTERVAL 365 DAY
...
async function build() {
  {
    ...
    return {
      ...
      referralK: (() => {
        const [r] = R.referralK || [];
        const [redemptions, baseWindow, baseAll] = (r || []).map((x) => Number(x) || 0);
        return {
      ...
      referralCohort: (R.referralCohort || []).map(([cohort, joined, referred]) => {
        ...
        return { cohort, joined: j, referredSomeone: rf, pct: j ? Math.round((rf / j) * 1000) / 10 : null };
      }),
