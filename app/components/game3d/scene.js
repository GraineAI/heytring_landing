...
export function createScene(canvas, opts = {}) {
  ...
  scene.fog = new THREE.Fog(0x000000, 90, 235);
  ...
  const stars = new THREE.Points(
    starGeo,
    keep(new THREE.PointsMaterial({ color: 0xffffff, size: 0.42, sizeAttenuation: true, transparent: true, opacity: 0.85, depthWrite: false }))
  );
  scene.add(stars);
  ...
  const grid = keep(new THREE.GridHelper(420, 60, PALETTE.tringDeep, 0x1c1c1e));
  ...
  const eBulletMat = basic(0xc77ddd, { blending: THREE.AdditiveBlending, depthWrite: false });
  ...
  const bossCore = new THREE.Mesh(keep(new THREE.IcosahedronGeometry(4.2, 1)), solid(0xc77ddd));
  boss.add(bossCore);
  ...
  function render(g, dt) {
    ...
    if (g.boss) {
      ...
      bossCore.material.color.setHex(b.hit > 0.4 ? 0xffffff : 0xc77ddd);
