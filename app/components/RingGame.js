  ...
  useEffect(() => {
        ...
        for (const f of G.foes) {
          ...
          if (b.x > f.x && b.x < f.x + f.w && b.y > f.y && b.y < f.y + f.h) {
            ...
            if (f.spam) {
              ...
              burst(f.x + f.w / 2, f.y + f.h / 2, "#FB7A5C");
    ...
    function draw() {
      ...
      for (const s of G.stars) {
        ...
        ctx.fillStyle = s.z > 1.4 ? "#FDEDE7" : "#8C7C73";
      ...
      for (const t of G.toasts) {
        ...
        ctx.fillStyle = t.bad ? "#FF7B72" : "#3FC98C";
...
function PilotBadge() {
  return (
    <svg width="104" height="104" viewBox="0 0 120 120" aria-hidden="true">
      ...
      <circle cx="49.6" cy="61.4" r="3.2" fill="#1B1512" />
      <circle cx="73.6" cy="61.4" r="3.2" fill="#1B1512" />
